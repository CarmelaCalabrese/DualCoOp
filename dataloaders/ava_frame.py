...
class AVA_ZSL(data.Dataset):
    def __init__(self, root, data_split, img_size=224, p=1, annFile="", label_mask=None, partial=1+1e-6):
        ann_file_names = {'train': 'train_all_labels_filtered.npy',
                          'val': 'val_all_labels_filtered.npy'}
        img_list_name = {'train': 'train_images_filtered.npy',
                         'val': 'val_images_filtered.npy',
                         'val_gzsl': 'val_gzsl_images_filtered_small.npy',
                         'test_gzsl': 'val_gzsl_images_filtered.npy'}
        self.root = root
        ...
        cls_id = pickle.load(open(os.path.join(self.root, 'annotations', 'zsl', "cls_id.pkl"), "rb"))
        if data_split == 'train':
            cls_id = cls_id['seen']
        elif data_split == 'val':
        ...
        self.image_list = np.load(image_list) #controlla che ci siano tutte le annotazioni per ogni immagine
