...
def main():
    ...
    for epoch in range(args.start_epoch, cfg.OPTIM.MAX_EPOCH):
        batch_time, losses, mAP_batches = train_coop(train_loader, [val_gzsi_loader], model, optim, sched, args, cfg, epoch,  cls_id)
        print('Train: [{0}/{1}]\t'
        ...
        if (epoch + 1) % args.val_every_n_epochs == 0 or epoch == args.stop_epochs - 1:
            ...
            print('Test: [{}/{}]\t '
                  ...
                  .format(epoch + 1, cfg.OPTIM.MAX_EPOCH,  p_gzsl, r_gzsl, f1_gzsl, mAP_gzsl), flush=True)
            print('Test: [{}/{}]\t '
                  ' P_gzsl {:.2f} \t R_gzsl {:.2f} \t F1_gzsl {:.2f} \t mAP_gzsl {:.2f}\t'
                  .format(epoch + 1, cfg.OPTIM.MAX_EPOCH, p_gzsl, r_gzsl,
